...
def wtite_pol_file(pol_dict, file_name):
    ...
    with open(file_name, "w") as data:
        data.write(text_str)
    print(f"Созданный многочлен : {text_str} записан в файл {file_name}")
...
k = abs(int(input("Введите натуральную степень первого многочлена: ")))

polynom_dist = get_pol_dict(k)
wtite_pol_file(polynom_dist, file_1)

k = abs(int(input("Введите натуральную степень второго многочлена: ")))
